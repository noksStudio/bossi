...
// ── מספרי ח.פ. / עוסק מורשה / ת.ז. ──────────────────────────────────────

// "ח.פ"/"ח״פ" (עסק) או "ת.ז"/"ת״ז"/"תעודת זהות" (אדם פרטי) — אותה
// תבנית עיצוב בדיוק לשתי התוויות, ואותו אלגוריתם ביקורת (למטה) לשתיהן.
// לקוח פרטי (לא חברה) מזוהה על סמך ת.ז. באותה צורה בדיוק שחברה מזוהה
// על סמך ח.פ. — ראו ADR-015 והשאלה שהוליכה לכאן ("האם זה עובד גם
// לפי ת״ז ועוד עבור לקוחות").
const ID_LABEL_RE = /ח\s?["״.]?\s?פ\.?|עוסק\s?(?:מורשה|פטור)|ת\s?["״.]?\s?ז\.?|תעודת\s?זהות/;
 ...
 * אמיתיים.
 *
 * מגבלה ידועה: ת.ז. אמיתית יכולה להיות קצרה מ-9 ספרות (האפס המוביל
 * מושמט לרוב בכתיבה חופשית). `NINE_DIGITS_RE` דורש בדיוק 9 — כמו
 * שהיה כבר נכון לח.פ., לא נפתר כאן, ראו ADR-015 לעיקרון הדומה.
 */
